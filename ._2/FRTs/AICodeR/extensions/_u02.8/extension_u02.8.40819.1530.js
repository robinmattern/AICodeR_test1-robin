     ...
     const  fs          =  require( 'fs' )
       var  FRT         =  require( './._2/FRTs/AICodeR_VSCodeExt/AIC90_FileFns_u03.js' );          // .(40820.02.1)
//     var  AIM         =  require( './AIC98_Apps-n-Models_u02.js' )                                //#.(40819.02.2)
       var  AIM         =  require( './._2/FRTs/AICodeR_VSCodeExt/AIC98_Apps-n-Models_u02.js' )     // .(40819.02.2)
       var  AIE         =  require( './._2/FRTs/AICodeR_VSCodeExt/AIC91_AppFolders_u03.js')         // .(40819.02.2 RAM Was: AIC91_ExtensionLib_u04.js)
...
//     var  getFilePath =  getFilePath_v02_8                                                        //#.(40819.04.x)
       var  getFilePath =  AIE.getFilePath                                                          // .(40819.04.x)
/*                                                                                                  //#.(40820.02.4 Beg)   
...
*/                                                                                                  //#.(40820.02.5 End)     
        if (bTest1) {  // Now you can use FRT, AIM, and AIE directly
            ...
            console.log(`  getFilePath('usermsg_'): ${await AIE.getFilePath('usermsg_')}`);
            return
            }
...
// AICodeR: 7. Open Response 'markdown.md', 0
       var  disposable = vscode.commands.registerCommand(      'extension.openResponse',     extension_openResponse )
            ...
            context.subscriptions.push( disposable );
  function  extension_openResponse() {      openFile_inVSCode( 'markdown.md', 0 ) }
...
// AICodeR: 9. Save Scripts 'markdown.md', 'save'
       var  disposable = vscode.commands.registerCommand(      'extension.saveScripts',      extension_saveScripts )
            console.log('  Congratulations, your extension,    "AICodeR: 8. Save Scripts", is now active!');
            context.subscriptions.push( disposable );
...
//  -----------------------------------------------------------------------------------------
  ...
  function  openFile_inVSCode( aType, nNew ) {
 //         console.log( "  getFilePath: ", String( getFilePath ) )
      ...
      var   aFound  = (fs_exists( aFilePath )) ? 'found' : 'not found'
            console.log( `  aFilePath: ${ aFilePath.replace( /.+docs[\\\/]/, '.\\docs' ) }, nNew: ${nNew}, ${aFound}` );
        if (bTest2) {  return }
        if (aFound == 'found') {
            vscode.workspace.openTextDocument( vscode.Uri.file( `${__basedir}/${aFilePath}` )).then( doc => {
            vscode.window.showTextDocument( doc );
            });
            aFile = aFile.replace( /[0-9]\.[0-9]{5}\.[0-9]{4}/, '..' )
            vscode.window.showInformationMessage( `  The file, '${aFile}', is ready to be edited!` );
        } else {
            vscode.window.showInformationMessage( `* The ${aType} file not found, '${aFilePath}!` );
            }
        };  // eof openFile_inVSCode
// -----------------------------------------------------------------------------------------

  function  runRequest_inVSCode( aType, nNew ) {
      var [ aFile, aFilePath ] =  getFilePath( aType, nNew )
      var   aFound  = (fs_exists( aFilePath )) ? 'found' : 'not found'
            console.log( `  aFilePath: ${ aFilePath.replace( /.+docs[\\\/]/, '.\\docs' ) }, nNew: ${nNew}, ${aFound}` );
        if (bTest2) {  return }
            aFile = aFile.replace( /[0-9]\.[0-9]{5}\.[0-9]{4}/, '..' )
        if (aFound == 'found' ) {
            ...
            vscode.window.showInformationMessage( `* Request file not found, '${aFilePath}!` );
            }
        }  // eof runRequest_inVSCode
// -----------------------------------------------------------------------------------------

  function  doScripts_inVSCode( aType, aWhat ) {
      var [ aFile, aFilePath ] =  getFilePath( aType, 0 )
      var   aFound  = (fs_exists( aFilePath )) ? 'found' : 'not found'
            console.log( `  aFilePath: ${ aFilePath.replace( /.+docs[\\\\]/, '.\\docs' ) }, aWhat: ${aWhat}, ${aFound}` );
        if (bTest2) {  return }
            aFile   =  aFile.replace( /[0-9]\.[0-9]{5}\.[0-9]{4}/, '..' )
        if (aFound == 'found' ) {
            ...
            vscode.window.showInformationMessage( `* Response file not found, '${aFilePath}!` );
            }
...
//function  getFilePath_v02_8( aType, bNew ) { ... }                                    // .(40819.02.1 RAM Rewrite).(40813.01.x).(40804.05.1 RAM Write getFilePath )
     async  function  getFilePath_v02_8( aType, bNew ) {                                //#.(40819.02.1 RAM Rewrite)..(40813.01.x).(40804.05.1 RAM Write getFilePath )
//     var  AIM         = (await import( './AIC98_Apps-n-Models_u02.mjs' )).default     //#.(40819.02.2)
...
//     var  mFile       =   getLastFile2( aAppName, aModel, '', aType, '', false )      // .(40819.04.1 RAM Use from AIE)
       var  mFile       =   AIE.getLastFile2( aAppName, aModel, '', aType, '', false )  // .(40819.04.1 RAM Use from AIE)
...
//  -----------------------------------------------------------------------------------------
  ...
  function  deactivate() {  // This method is called when your extension is deactivated
        if (vscode) { console.log( "  deactivating extension" ); return }
        if (bTest1 || bTest2) { process.exit() }                                             // .(40819.09.1 RAM Move it here)
            }  //  eof deactivate
// -----------------------------------------------------------------------------------------
