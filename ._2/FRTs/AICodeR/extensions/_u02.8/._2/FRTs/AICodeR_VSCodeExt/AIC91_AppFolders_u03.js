...
// --- ---  --------------  =  -------------------------------------------------------------
  ...
  function  getLastFile2( aApp_, aMod_, aVer, aType, aExt, bQuiet ) {                               // .(40905.01.1).(40828.01.1 RAM aVer Was: aToday).(40801.09.5)
       var  aErrFile        = `docs/${aApp_}/${aMod_}/c${aApp_.slice(0,2)}_t###.##.#.#####.####_${aType}` 
       var  nFld            = (aMod_.length == 7) ? 1 : 2; aApp = aApp_.slice(0,3)                  // .(40905.01.2 RAM Save passed Appname amd Model)
       var  aAppName        = (getApp(    2,   aApp    , 2 ) ) // (().slice(2,3)[0] || '').trim()   // .(40718.09.5).(40715.01.3 RAM Was 2, aApp)
        if (aAppName == '') {  return [ `* App, '${  aApp_}', not found`, `${aErrFile}.${aExt}` ] } // .(40905.01.3 RAM if not in Apps list)
       var  aModel          = (getModel( nFld, aMod_   , 2 )) // (().slice(2,3)[0] || '').trim()    // .(40718.09.6).(40715.01.4 RAM Was 2, aMod)
        if (aModel   == '') {  return [ `* Model, '${aMod_}', not found`, `${aErrFile}.${aExt}` ] } // .(40905.01.4 RAM if not in Models list)
...
//  return [ `* AppName/Model folder, ./${docs/${aApp_}/${aMod_}}`, `docs/${aApp_}/${aMod_}/c##_t###.##.#.#####.####_${aType}.${aExt}`] }  //#.(40907.03.3 RAM Same error as above)
